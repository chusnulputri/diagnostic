                ...
                <div class="ibox float-e-margins">
                    <div class="ibox-title" style="border-top-left-radius: 10px; border-top-right-radius: 10px; padding: 20px 30px; border: 0px; border-bottom: 2px solid #f5f5f5;">
                        <span style="font-size: 16pt; font-weight: 600; color: var(--sidebarActiveBg);">
                            <i class="fa fa-file-medical"></i>&nbsp; Rule
                        </span>
...
@csrf
@include('Admin::partials.modal-tambah-rule')
@include('Admin::partials.modal-edit-rule')
@endsection
    ...
    const vue = new Vue({
        ...
        methods: {
            getServerData: function(){
                ...
                axios.get("{{Route('admin.get_data_rule')}}")
                    .then(response => {
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                $('#modalEditRule').modal('show');
                $('[name="r_penyakit_id"]').val(rule.r_penyakit_id);
                $('[name="r_gejala_id"]').val(rule.r_gejala_id);
                $('#formUpdateRule').attr('action', response.update_url)
            }
        });
    });
